...
CanWrapper::CanWrapper(){
    // constructor of the CANWrapper class
    m_initialized = false;
...
bool CanWrapper::Init(const char *interfaceName, int &errorCode)
{
    ...
    // SOCK_RAW: Raw mode is basically there to allow to bypass some of the way that the computer handles TCP/IP (Rather than going through the normal layers of encapsulation/decapsulation that the TCP/IP stack on the kernel does, you just pass the packet to the application that needs it.)
    // SOCK_NONBLOCK: non blocking socket (while receiving for instance)
    // CAN_RAW: bidding stuff...
    ...
    // if we arrive there, everything went well, the socket is initialized correctly (hopefully)
    m_initialized = true;
    return true;
}
...
bool CanWrapper::SendMsg(struct can_frame msg, bool extended, bool rtr, int &errorCode)
{
    ...
    if(!m_initialized){
        errorCode = -1;
        return false; // do not try to send something if the socket has not been initialized
    }
...
// Get a CAN message. As the socket is non blocking, it will return false if there is no data or if there is any error.
...
int CanWrapper::EnableErrorMessages()
{
    int ret;
